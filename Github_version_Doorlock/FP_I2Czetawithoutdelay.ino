...
void loop()         
{  
  if(! Serial.available())//SEARCH. If no instruction from upper system(PC), then enter default mode and scan for fingers. (LED on FP module continuously blinking). if finger is placed, tries to find a match.
  {
    Serial.println("Waiting for valid finger...");
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("   Place Your");lcd.setCursor(0, 1);lcd.print("Enrolled Finger");
...
uint8_t getFingerprintID() {
  ...
  lcd.clear();
  lcd.setCursor(0,0);
  for(int i=0;i<10000&&!Serial.available();i++);                                                          //Wait for a username ( First name) from upper system(PC).if available within 10000 for loops, 
  username = Serial.readString();                                                                         //displays it. Otherwise, simply prints ACCESS GRANTED.    
